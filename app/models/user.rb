class User < ActiveRecord::Base
  ...
  def send_notification(alert, drive_id, state)
    APNS.host = 'gateway.sandbox.push.apple.com'
    APNS.pem = Rails.root.join('config/dev_push.pem')
    APNS.port = 2195
    APNS.send_notification(self.devise_token, alert: alert, badge: 1, sound: 'default', other: { drive_id: drive_id, state: state })
